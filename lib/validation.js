...
// Shared input validation for all API routes. Returns { valid, errors }.
...
export function validateGenerationEventInput(body) {
  const errors = [];
  if (!body || typeof body !== "object") {
    return { valid: false, errors: ["Request body must be a JSON object."] };
  }
  ...
  if (!OUTCOMES.includes(body.outcome)) {
    errors.push(`outcome is required and must be one of: ${OUTCOMES.join(", ")}.`);
  }
  if (body.errorReason !== undefined && body.errorReason !== null && typeof body.errorReason !== "string") {
    errors.push("errorReason must be a string if provided.");
  }
  if (body.wordListId !== undefined && body.wordListId !== null && typeof body.wordListId !== "string") {
    errors.push("wordListId must be a string if provided.");
  }
  return errors.length ? { valid: false, errors } : { valid: true };
}

export function validatePageViewInput(body) {
  const errors = [];
  if (!body || typeof body !== "object") {
    return { valid: false, errors: ["Request body must be a JSON object."] };
  }
  ...
  if (!Number.isFinite(body.durationMs) || body.durationMs < 0) {
    errors.push("durationMs is required and must be a non-negative number.");
  }
  return errors.length ? { valid: false, errors } : { valid: true };
}
